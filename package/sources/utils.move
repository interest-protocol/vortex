...
public(package) macro fun to_field_element_bytes<$T>($value: $T): vector<u8> {
    ...
    let mut field_value = bytes_to_u256!(value_bytes);
    
    // Ensure the value is within the field (reduce modulo the prime)
    if (field_value >= 21888242871839275222246405745257275088548364400416034343698204186575808495617) {
        field_value = field_value % 21888242871839275222246405745257275088548364400416034343698204186575808495617;
    };
    
    // Convert back to 32-byte big-endian representation
    u256_to_32_bytes_be!(field_value)
}

// Helper function to convert bytes to u256 (assumes big-endian input)
public(package) macro fun bytes_to_u256($bytes: vector<u8>): u256 {
    let bytes = $bytes;
    let mut result: u256 = 0;
    let mut i = 0;
    let len = bytes.length();
    
    while (i < len && i < 32) {  // Max 32 bytes for u256
        result = result << 8;
...
public(package) macro fun u256_to_32_bytes_be($value: u256): vector<u8> {
    let value = $value;
    ...
    let mut i = 0;
    while (i < 32) {
        bytes.push_back(((temp >> (8 * (31 - i))) & 0xFF) as u8);
        i = i + 1;
    };
