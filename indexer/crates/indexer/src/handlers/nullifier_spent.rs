        ...
        for tx in &checkpoint.transactions {
            ...
            for (idx, ev) in events.data.iter().enumerate() {
                ...
                if ev.type_.module.as_str() != "vortex_events"
                    || ev.type_.name.as_str() != "NullifierSpent"
                {
                    continue;
                }

                let event = match bcs::from_bytes::<NullifierSpentEvent>(&ev.contents) {
                    Ok(e) => e,
                    Err(e) => {
                        warn!(
                            ...
                            event_idx = idx,
                            error = %e,
                            "Failed to deserialize NullifierSpent event"
                        );
                        continue;
                    }
                };
